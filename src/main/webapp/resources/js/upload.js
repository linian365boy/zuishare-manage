	var initUpload = function() {
		...
		uploader = WebUploader.create({
			...
			chunked : false,
			chunkSize : 512 * 1024,
			server : basePath+'admin/img/upload.html',
			// runtimeOrder: 'flash',
			accept: {
			 title: 'Images',
		...
		uploader.on( 'uploadSuccess', function( file,response) {
        	...
        	if(response.code==200){
        		//为图片添加选中事件
        		$(".typeclassb").append('<input type="hidden" value="'+response.obj+'" name="imageContent"/>')
        	}else{
        		//$("#"+file.id).attr("class","state-error");
        		//$("#"+file.id).append( '<p class="error">上传失败</p>' );
            	if (response.code == 201) {
            		errorText =errorText+file.name+response.message+'<br>';
		...
		uploader.onError = function(code) {
			if(code=="F_EXCEED_SIZE"){
        		art.dialog.alert("图片超出指定大小");
        	}else if(code=="F_DUPLICATE"){
        		art.dialog.alert("图片重复添加");
        	}else{
        		art.dialog.alert( 'Eroor: ' + code );
        	}
		};
		...
		$info.on('click', '.ignore', function() {
			var errorImgs = uploader.getFiles('error')
			$.each(errorImgs,function(i,n){
				removeFile(n);
			})
		});
